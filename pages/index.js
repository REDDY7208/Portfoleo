...
const Home = () => {
  ...
  return (
    <div className='bg-primary/60 h-full'>
      ...
      <button
        onClick={toggleMusic}
        className='fixed top-24 right-8 z-50 bg-accent/20 hover:bg-accent/40 p-3 rounded-full border border-accent/50 transition-all duration-300'
        aria-label='Toggle background music'
      >
        {isPlaying ? (
          <HiVolumeUp className='text-2xl text-white' />
        ) : (
          <HiVolumeOff className='text-2xl text-white' />
        )}
      </button>
